...
extension SdkConfigurationManager {
    static var selphIDConfiguration: SelphIDConfigurationData {
        ...
        let docType: SelphIDDocumentType = PrefManager.get(.ID_CARD, forKey: .KEY_NFC_DOCUMENT_TYPE)
        
        configSelphID.wizardMode = true
        configSelphID.showResultAfterCapture = true
        configSelphID.specificData = "ES|<ALL>"
        configSelphID.tokenImageQuality = 0.5
        configSelphID.scanMode = docType == .PASSPORT ? .MODE_GENERIC: .MODE_SEARCH
        configSelphID.documentType = docType
